...
def write_tif(filename, tiffarray):
    num_frames, height, width = tiffarray.shape
    ...
    for i in range(num_frames):
        ...
        frame = np.transpose(tiffarray[i, :, :])

        # 将每个帧从NumPy数组转换为PIL图像
...
def read_array(filename, dset_name='dataset'):
    ...
    if os.path.isdir(filename):
        ...
        for file in files:
            if os.path.isfile(file):
                array = read_array(file, dset_name)
    ...
    elif data_format[1] in ('.mrc', '.map', '.rec'):
        array = read_mrc(filename)
    elif data_format[1] in ('.tif', '.TIF'):
        array = read_tif(filename)
    elif data_format[1] == '.png':
        array = read_png(filename)
    else:
        print('/!\ DeepFinder can only read .h5, .mrc, .tif, and .png formats')
        return None
    return array
...
def write_array(array, filename, dset_name='dataset'):
    ...
    if data_format[1] == '.h5':
        ...
        write_png(array, filename)
    elif data_format[1] == '.tif':
        write_tif(array, filename)
    else:
        print('/!\ DeepFinder can only write arrays in .h5, .mrc, .png, and .tif formats')
